...
case class Coordinates3(className: String, method: String, version: String, serializer: Serializer):
...
case class Coordinates2(className: String, method: String, version: String):
  def withSerializer(serializer: Serializer): Coordinates3 = Coordinates3(className, method, version, serializer)

object Coordinates3:
  def apply(coordinates2: Coordinates2, serializer: Serializer): Coordinates3 =
    Coordinates3(coordinates2.className, coordinates2.method, coordinates2.version, serializer)

  def apply(coords: String): Coordinates3   = coords.split(':') match
    case Array(className, method, version, serializer) =>
      Coordinates3(className, method, version, Serializer.valueOf(serializer))
