...
@main def generatorApp() =
  ...
  val codes = CallerProxy.builder().withAvroBinarySerialization.generateCode(tastyFiles) ++ ReceiverGenerator.apply().apply(StructureExtractor.apply().apply(tastyFiles))
